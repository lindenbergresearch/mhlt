...
void GetParamsFromEnt(entity_t *mapent) {
    ...
    sprintf(szTmp, "%i", g_max_map_miptex);
    ...
    for (hull = 0; hull < NUM_HULLS; hull++) {
        ...
        for (bn = 0; bn < e->numbrushes; bn++) {
            ...
            for (; f; f = next) {
                ...
                if (bh2->bounds.testDisjoint(f->bounds)) {// this face doesn't intersect brush2's bbox
                    f->next = outside;
                    outside = f;
                    continue;
                }
                ...
                if (f) {
                    ...
                    if (b1->contents > b2->contents) {// inside a water brush
                ...
                } else {// the entire thing was on the outside, even
...
static void CheckForNoClip() {
    ...
    for (i = 0; i < g_numentities; i++) {
        ...
        if (!strncasecmp(entclassname, "func_illusionary", 16)) {
        ...
        //  func_door_rotating or func_tracktrain (passable, not-solid flag )
                 ...
                 (/* NOTE: func_doors as far as i can tell may need clipnodes for their
                  ...
                  (!strncasecmp(entclassname, "func_train", 10)) || (!strncasecmp(entclassname, "func_door", 9))
