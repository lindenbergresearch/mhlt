...
int FindIntPlane(const vec_t *const normal, const vec_t *const origin) {
  ...
  while (1) {
    ...
    if (fabs(t) < DIST_EPSILON) {                                                  // on plane
...
const char *ContentsToString(const contents_t type) {
  switch (type) {
    ...
    case CONTENTS_HINT:return "HINT";

#ifdef ZHLT_NULLTEX // AJM
    case CONTENTS_NULL:return "NULL";
#endif
...
contents_t CheckBrushContents(const brush_t *const b) {
  ...
  if ((b->entitynum==0) || (strcmp("func_group", ValueForKey(&g_entities[b->entitynum], "classname"))==0)) {
    ...
    switch (contents) {
      ...
      case CONTENTS_CLIP:
#ifdef ZHLT_NULLTEX // AJM
      case CONTENTS_NULL:break;
#endif
      default:Fatal(assume_BRUSH_NOT_ALLOWED_IN_ENTITY, "Entity %i, Brush %i: %s brushes not allowed in entity", b->entitynum, b->brushnum, ContentsToString(contents));
        break;
    }
  }
