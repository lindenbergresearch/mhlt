...
bool NetvisSocket::OnAccept(const ost::InetHostAddress &ia, ost::tpport_t port) {
  Log("Connection accepted from %s\n", ia.getHostname());
  return true;
}

NetvisSocket::NetvisSocket(const ost::InetAddress &ia, ost::tpport_t port)
    : TCPSocket(ia, port) {
}
...
void NetvisSession::Run() {
  ...
  while (_zhlt_isCancelled()==false) {
    if (m_TCPStream.isPending(ost::SOCKET_PENDING_ERROR, 0)) {
      goto FatalHandler;
    } else if (m_TCPStream.isPending(ost::SOCKET_PENDING_INPUT, 0)) {
...
NetvisSession::NetvisSession(NetvisSocketServer *parent, ost::TCPSocket &server)
...
NetvisSession::NetvisSession(const ost::InetHostAddress &host, ost::tpport_t port)
...
void NetvisSession::DisplayClientStats() {
  ...
  Log(
      ...
      "==================================================\n", g_clientid, g_visportals*2, g_serverindex, serverpercent, (int) m_PortalCount, clientpercent, total_seconds, work_seconds, idle_seconds);
}
...
#ifdef SYSTEM_POSIX
      if (socket->getErrorNumber() == ost::SOCKET_BINDING_FAILED)
      {
          Sleep(2000);
...
NetvisSocketServer::NetvisSocketServer(ost::tpport_t port) {
  m_Port = port;
}
...
void NetvisSocketServer::DisplayServerStats() {
  ...
  Log("\n"
      ...
      "System Progress   : [%-5d portals] (%3.2f percent)\n"
      "Server Progress   : [%-5d portals] (%3.2f percent)\n", g_visportals*2, g_PortalArrayIndex, globalpercent, g_vislocalportal, localpercent);
