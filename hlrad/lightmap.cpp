  ...
  Developer(DEVELOPER_LEVEL_SPAM,
    ...
    pLuxelFlags = LuxelFlags;
    {
        ...
        for (t = 0; t < h; t++) {
            for (s = 0; s < w; s++, surf += 3, pLuxelFlags++) {
                if (!*pLuxelFlags) {
                    int x_min = Max(0, s - 1);//SSS min -> Min & max -> Max
                    int x_max = Min(w, s + 1);
                    int y_min = Max(0, t - 1);
                    int y_max = Min(t, t + 1);
...
void CreateDirectLights() {
    ...
    for (i = 0; i < (unsigned) g_numentities; i++) {
        ...
        if (!strcmp(name, "light_spot") || !strcmp(name, "light_environment") || target[0]) {
            ...
            if (!VectorAvg(dl->intensity)) VectorFill(dl->intensity, 300);
            dl->type = emit_point;
        }
            ...
            if (((l->type == emit_skylight) && (g_sky_lighting_fix)) || (pvs[(i - 1) >> 3] & (1 << ((i - 1) & 7)))) {
                for (; l; l = l->next) {
...
#ifdef HLRAD_HULLU
                        vec3_t transparency = {1.0, 1.0, 1.0};
                        if (TestSegmentAgainstOpaqueList(pos, delta, transparency))
#else
                    ...
                    if (VectorMaximum(add) > (l->style ? g_coring : 0)) {
                        ...
                        if (l->type != emit_skylight) {// Don't test from light_environment entities to face, the special sky code occludes correctly
#ifdef HLRAD_HULLU
                            if (TestSegmentAgainstOpaqueList(pos, l->origin, transparency))
#else
