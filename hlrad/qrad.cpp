...
void GetParamsFromEnt(entity_t *mapent) {
  ...
  if (pszTmp) {
    ...
    if (sscanf(pszTmp, "%f %f %f", &red, &green, &blue)) {
      ...
      if (red==0 && green==0 && blue==0) {} // dont bother setting null values
      else {
        g_ambient[0] = red*128;
...
static void BounceLight() {
  ...
  for (i = 0; i < g_numbounce; i++) {
    printf("Bounce %u ", i + 1);
#ifdef HLRAD_HULLU
    if (g_rgb_transfers) {NamedRunThreadsOn(g_num_patches, g_estimate, GatherRGBLight); }
    else {NamedRunThreadsOn(g_num_patches, g_estimate, GatherLight); }
#else
...
static void CheckMaxPatches() {
  switch (g_method) {
    case eMethodVismatrix:hlassume(g_num_patches < MAX_VISMATRIX_PATCHES, assume_MAX_PATCHES);
      break;
    case eMethodSparseVismatrix:hlassume(g_num_patches < MAX_SPARSE_VISMATRIX_PATCHES, assume_MAX_PATCHES);
      break;
...
static void MakeScalesStub() {
  switch (g_method) {
    case eMethodVismatrix:MakeScalesVismatrix();
      break;
    case eMethodSparseVismatrix:MakeScalesSparseVismatrix();
      break;
    case eMethodNoVismatrix:MakeScalesNoVismatrix();
      break;
  }
}
...
static void Settings() {
  ...
  switch (g_threadpriority) {
    ...
    default:tmp = "Normal";
      break;
    case eThreadPriorityLow:tmp = "Low";
      break;
    case eThreadPriorityHigh:tmp = "High";
      break;
  }
  ...
  switch (g_method) {
    default:tmp = "Unknown";
      break;
    case eMethodVismatrix:tmp = "Original";
      break;
    case eMethodSparseVismatrix:tmp = "Sparse";
      break;
    case eMethodNoVismatrix:tmp = "NoMatrix";
      break;
  }
